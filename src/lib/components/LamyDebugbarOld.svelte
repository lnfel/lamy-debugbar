<script>
    import { renderToHtml, BUNDLED_THEMES, BUNDLED_LANGUAGES } from 'shiki-es'
    ...
    /**
     * @type {import('shiki-es').Highlighter}
     */
    let ShikiHighlighter
    let ready = false

    /**
     * @type {Promise<ArrayLike<unknown[]>>}
     */
    let render = new Promise((resolve) => resolve([]))

    onMount(async () => {
        ShikiHighlighter = await shikiHighlighter(highlighter)
    })

    $: {
        async function shikiWatcher() {
            if (ShikiHighlighter) {
                console.log('ShikiHighlighter: ', ShikiHighlighter)
                console.log('current highlighter.theme: ', highlighter.theme, theme)
                console.log('Loaded themes: ', ShikiHighlighter.getLoadedThemes())
                console.log('isThemeLoaded: ', ShikiHighlighter.getLoadedThemes().includes(theme))
                if (!ShikiHighlighter.getLoadedThemes().includes(theme)) {
                    console.log(`${highlighter.theme} is not loaded, loading theme.`)
                    await loadTheme(theme)
                } else {
                    console.log('Theme is already loaded, syncing colors.')
                    console.log('getTheme: ', ShikiHighlighter.getTheme(highlighter.theme))
                    syncColors()
                }
                ...
                render = new Promise(async (resolve, reject) => {
                    for (const [key, value] of Object.entries(data)) {
                        if (htmlArray instanceof Array) {
                            htmlArray.push(await renderShiki(value))
                        }
                    }
                    resolve(htmlArray)
                    lamyIsReady()
    ...
    function syncColors() {
        const pre = document.querySelectorAll('.shiki')
        pre.forEach((element) => {
            if (element instanceof HTMLElement) {
                element.style.backgroundColor = ShikiHighlighter.getBackgroundColor(ShikiHighlighter.getTheme(highlighter.theme).name)
            }
        })
    }
    ...
    const renderShiki = async (code) => {
        ...
        const tokens = ShikiHighlighter.codeToThemedTokens(JSON.stringify(code, null, 2), 'javascript')

        console.log('ShikiHighlighter.getTheme(highlighter.theme).name: ', ShikiHighlighter.getTheme(highlighter.theme).name)
        return renderToHtml(tokens, {
            // bg: shiki?.highlighter.getBackgroundColor(highlighter?.theme ?? 'material-palenight'),
            // highlighter.theme
            bg: ShikiHighlighter.getBackgroundColor(ShikiHighlighter.getTheme(highlighter.theme).name ?? 'material-palenight'),
            elements: {
    ...
    const switchTab = async (event) => {
        if (ShikiHighlighter) {
             ...
             */
            const activeTabColor = ShikiHighlighter.getTheme(highlighter.theme ?? 'material-palenight').colors?.["terminal.ansiBlue"] ?? ''
            const foregroundColor = ShikiHighlighter.getForegroundColor(ShikiHighlighter.getTheme(highlighter.theme).name ?? 'material-palenight')
    ...
    <details class="lamy-details relative overflow-hidden" style="{ready ? 'display: block;' : 'display: none;'}" bind:open>
        <div class="max-h-96 border-t border-gray-700 overflow-x-hidden overflow-y-auto">
            ...
            <!-- {#if ShikiHighlighter} -->
                ...
                {:then htmlArray}
                    {#each htmlArray as shikiHTML, i}
                        <div class="lamy-tab-content {Object.keys(data)[i]} {i === 0 ? '' : 'hidden'}">{@html shikiHTML}</div>
                    {/each}
                {/await}
            <!-- {/if} -->
        </div>
        
        {#if ShikiHighlighter?.getLoadedThemes().includes(theme) && ready}
        <summary
            class="lamy-toggle flex items-center justify-between list-none outline-none"
            style="
                { ShikiHighlighter ? `background-color: ${ShikiHighlighter.getBackgroundColor(ShikiHighlighter.getTheme(highlighter.theme).name ?? 'material-palenight')};` : '' }
                { ShikiHighlighter ? `color: ${ShikiHighlighter.getForegroundColor(ShikiHighlighter.getTheme(highlighter.theme).name ?? 'material-palenight')}` : '' }
            ">
            <!--
            ...
            <div class="flex items-center min-w-0">
                ...
                <div>
                    {JSON.stringify(ShikiHighlighter.getTheme(highlighter.theme).name)}
                </div>
                <div class="lamy-tablist flex overflow-x-auto">
                    {#each Object.keys(data) as key, i (`${ShikiHighlighter.getTheme(highlighter.theme).name}${i}`)}
                        <button on:click={switchTab} id={key} type="button" class="lamy-tab whitespace-nowrap px-2 py-2 outline-none hover:bg-gray-600 focus:bg-gray-600"
                            style="{ i === 0 ? `color: ${ShikiHighlighter?.getTheme(highlighter?.theme ?? 'material-palenight').colors?.["terminal.ansiBlue"]}` : '' }">
                            { key }
                        </button>
                    {/each}
                </div>
